...
def train_and_evaluate(args, alpha, beta):
    ...
    log_dir = os.path.join(weight_dir, 'logs_tensorboard')

    G_opt = optim.Adam(model.parameters(), lr=args.lr)
    
    # lr scheduler
    # scheduler = optim.lr_scheduler.StepLR(G_opt, step_size=20, gamma=0.5)
    ...
    for epoch in range(lastepoch,args.num_epoch):
        ...
        # for i, databatch in enumerate(tqdm(train_loader, desc=f"Epoch {epoch} 训练")):
        ...
        if epoch % 5 == 0:  # 每10个epoch验证一次
            ...
            with torch.no_grad():
                ...
                for i, databatch in tqdm(enumerate(test_loader), total=total_batches, desc=f"Epoch {epoch} 验证"):
                    ...
                    pred_rgb = pred_rgb.squeeze(0).cpu().numpy().astype(np.uint8)#pred_rgb.transpose(1, 2, 0)
                    gt_rgb =  gt_rgb.round()
                    gt_rgb = gt_rgb.squeeze(0).cpu().numpy().astype(np.uint8)#pred_rgb.transpose(1, 2, 0)
                    skpsnr += peak_signal_noise_ratio(pred_rgb.transpose(1, 2, 0),gt_rgb.transpose(1, 2, 0), data_range=255)
                    skssim += structural_similarity(pred_rgb.transpose(1, 2, 0),gt_rgb.transpose(1, 2, 0), win_size=11, data_range=255, channel_axis=2, gaussian_weights=True)
            ...
            skpsnr = skpsnr / val_count
            skssim = skssim / val_count
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_dir', type=str, default='/root/shared-nvme/zpc/SOCODE/MAI')
    parser.add_argument('--train_list_file', type=str, default='/root/shared-nvme/zpc/SOCODE/MAI/data/ISP/AAAI-25/MAI_dataset/train.txt')
    parser.add_argument('--test_list_file',  type=str, default='/root/shared-nvme/zpc/SOCODE/MAI/data/ISP/AAAI-25/MAI_dataset/val.txt')
    
    parser.add_argument('--train_datatype', type=str, default='train')
    parser.add_argument('--test_datatype',  type=str, default='test')
    parser.add_argument('--result_dir', type=str, default='/root/shared-nvme/zpc/SOCODE/Results/result_MSO_ECCA/MAI/L1_WaveLoss_sum_weights')
