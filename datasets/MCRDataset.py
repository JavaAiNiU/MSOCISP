...
class MCRDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        input_raw = imageio.imread(os.path.join(self.data_dir, input_path))
        gt_raw = imageio.imread(os.path.join(self.data_dir, gt_raw_path))
        gt_rgb = imageio.imread(os.path.join(self.data_dir, gt_rgb_path))

        # 处理 input_raw
        if input_raw.ndim == 2:
            input_raw = np.expand_dims(input_raw, axis=0)
        elif input_raw.ndim == 3:
            input_raw = input_raw.transpose(2, 0, 1)
        else:
            raise ValueError(f"Unexpected number of dimensions {input_raw.ndim} for input image at {os.path.join(self.data_dir, input_path)}")

        # 处理 gt_raw
        if gt_raw.ndim == 2:
            gt_raw = np.expand_dims(gt_raw, axis=0)
        elif gt_raw.ndim == 3:
            gt_raw = gt_raw.transpose(2, 0, 1)
        else:
            raise ValueError(f"Unexpected number of dimensions {gt_raw.ndim} for gt_raw image at {os.path.join(self.data_dir, gt_raw_path)}")

        # 处理 gt_rgb
        if gt_rgb.ndim == 2:
            gt_rgb = np.expand_dims(gt_rgb, axis=0)
        elif gt_rgb.ndim == 3:
            gt_rgb = gt_rgb.transpose(2, 0, 1)
        else:
            raise ValueError(f"Unexpected number of dimensions {gt_rgb.ndim} for gt_rgb image at {os.path.join(self.data_dir, gt_rgb_path)}")
...
if __name__ == '__main__':
    dataset = MCRDataset(data_dir='/raid/hbj/datas/MCR', image_list_file='MCR_test_list.txt', data_type='test',
                         patch_size=None)
    data = dataset[0]
